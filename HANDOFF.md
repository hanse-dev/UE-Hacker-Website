...
> Aktueller Branch `projekt-caesar-eigener-code` (3.61, Experiment, noch nicht gemergt): Cäsar-Chiffre
> testweise auf "selbst schreiben + Lösung auf Wunsch" umgestellt.
> `npm run test:checks` grün (345 Tests).
...
| 3.61 | Experiment: Cäsar-Chiffre auf "selbst schreiben + Lösung auf Wunsch" umgestellt (Branch `projekt-caesar-eigener-code`, ungemergt) | `codeTemplate` blankt den Funktionskörper nur, wenn die Funktion in der Lektion neu eingeführt wird (Lektion 3); schon vorher gebaute Funktionen bleiben als Gerüst (Lektion 4/5). Jede Aufgabe hat ein `solution`-Feld (mit `python3` geprüft); `LessonView.vue` zeigt es erst nach Klick auf "Lösung anzeigen" (`t()`-Keys `lesson.showSolution`/`solutionLabel`), Box verschwindet bei erledigter Aufgabe. Test: `tests/site.spec.js` "Lösung wird erst nach Klick...". **Offen, falls überzeugend:** gleiches Muster auf Vigenère/Snake/Text-Adventure übertragen. |
