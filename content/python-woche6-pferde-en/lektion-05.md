# 🔍 Lesson 5: Searching and counting

```python
horses = ["Stormwind", "Lightning", "Stormwind", "Luna", "Stormwind", "Fox"]
print("Lightning" in horses)          # True – is the entry in there?
print("Nova" in horses)          # False
print(horses.index("Lightning"))      # position of the first match
print(horses.count("Stormwind"))      # how often does it occur?
```

- **`x in list`** gives `True` or `False` – ideal for `if`
- **`list.index(x)`** returns the **position** of the first match (if `x` is not there, you get an error – check with `in` first)
- **`list.count(x)`** counts **how often** `x` occurs
- The word **`in`** works with text too: `"Gold" in "Goldcoin"` is `True`
