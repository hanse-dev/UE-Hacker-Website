# Knowledge Cheat Sheet: Week 12

_Summary of all topics from Weeks 1–11_
...
### import
Load ready-made functions and constants from modules.
...
### math module
Mathematical functions: square root, power, sine, pi, etc.
...
### Aliases
Import modules with a custom short name.
...
### Dictionaries
Key-value pairs. Access by key instead of index.
...
### Tuples
Immutable sequence – created with `()`. Cannot be changed.
...
### Immutability
Tuple elements cannot be replaced. Good for fixed data like coordinates.

---

## Week 9: JSON files and I/O

### Files with open()
Read and write files. Always use `with` for automatic closing.
...
### JSON
Save and load dictionaries and lists as files.
...
### CSV
Tabular data – read/write rows. Use the `csv` module.
...
### Classes and objects
Class = blueprint. Object = concrete instance. Bundle attributes and methods together.
...
Attributes are the object's data. Methods are functions that operate on the object. `self` refers to the object itself.

### Instantiation
Create a new object with `ClassName()`. `__init__` is called automatically.
...
### Inheritance
Child classes inherit attributes and methods from the parent class. Reuse code.
...
### Polymorphism
Same method name, different behaviour depending on the class. Flexible systems.

### Magic methods
Special methods with `__name__`: `__init__`, `__str__`, `__len__` etc. Python calls them automatically.
...
Inheritance = "is-a". Composition = object contains other objects ("has-a").
