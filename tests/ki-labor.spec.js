...
const PROGRESS_KEY_WEEK8 = 'ue-hacker-interactive-progress-ki-labor-woche8';
...
test.describe('KI-Labor (Wochenauswahl)', () => {
  ...
  test('Wochenauswahl: 8 Kacheln, Woche 1-5 + 8 verfügbar, Woche 6-7 "kommt noch"', async ({ page }) => {
    // Wochen 6+7 (Neuronale Netze I/II) sind auf separaten Branches in Arbeit und in diesem
    // Arbeitsstand noch nicht vorhanden - Woche 8 (letzte Woche des Kurses) ist unabhängig davon
    // bereits fertig. Nach dem Mergen aller drei Wochen-Branches sind hier alle 8 Wochen
    // verfügbar; dieser Test spiegelt dann nur noch den Zwischenstand dieses Branches.
    ...
    await expect(page.locator('.week-tile').nth(7)).not.toBeDisabled();
  ...
  test('Deep-Link ?week=8 öffnet direkt die Wochen-Tour (letzte Woche des Kurses)', async ({ page }) => {
    await page.goto('/kurs/ki-labor?week=8');
    // 10 Lektionen (5 Lektion + Debug + Mission + 3 Extra-Herausforderung) + 1 Check-Punkt, da
    // content/ki-labor-checks/week-8.json existiert (hasCheck=true).
    await expect(page.locator('.stepper-step')).toHaveCount(11, { timeout: 15000 });
    await expect(page.locator('.tour-breadcrumb')).toContainText('Woche 8');
    await expect(page.locator('.tour-breadcrumb')).toContainText('Grenzen & Ethik');
  });
  ...
  test('Woche 8: Debug-Lektion mit fehlender Zaehlung/umgedrehtem Vergleich/falschem Filter', async ({ page }) => {
    await page.addInitScript((key) => {
      localStorage.setItem(key, JSON.stringify({
        version: 1,
        completedLessonIds: ['lektion-01', 'lektion-02', 'lektion-03', 'lektion-04', 'lektion-05'],
  ...
  test('Kompletter Durchlauf Woche 8: alle Lektionen, Extra-Herausforderungen und der Wochen-Check', async ({ page }) => {
    test.setTimeout(120000);
    await page.goto('/kurs/ki-labor?week=8');
    await initKernel(page);

    // Lektion 1: Wenn Trainingsdaten schief sind.
    await solve(page, [null, null, 'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\nnachrichten = [\n    {"grossbuchstaben_anteil": 4, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 8, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 12, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 16, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 20, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 24, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 28, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 32, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 18, "kategorie": "spam"},\n    {"grossbuchstaben_anteil": 22, "kategorie": "spam"},\n]\nprint(anzahl_je_klasse(nachrichten, "kategorie"))\n']);
    await page.locator('.btn-next').click();
    await expect(page.locator('.progress-count')).toContainText('1 abgeschlossen');

    // Lektion 2: Der Trick mit der Genauigkeit.
    await solve(page, [null, null, 'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\nnachrichten = [\n    {"grossbuchstaben_anteil": 4, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 8, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 12, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 16, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 20, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 24, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 28, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 32, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 18, "kategorie": "spam"},\n    {"grossbuchstaben_anteil": 22, "kategorie": "spam"},\n]\nbasis = mehrheitsklasse(nachrichten, "kategorie")\nprint(genauigkeit_je_klasse(nachrichten, "kategorie", lambda b: basis))\n']);
    await page.locator('.btn-next').click();
    await expect(page.locator('.progress-count')).toContainText('2 abgeschlossen');

    // Lektion 3: Ein Baum lernt die Schieflage mit.
    await solve(page, [null, 'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\ndef teile_bei_schwellenwert(daten, merkmal, schwelle):\n    links = [b for b in daten if b[merkmal] < schwelle]\n    rechts = [b for b in daten if b[merkmal] >= schwelle]\n    return links, rechts\n\ndef anzahl_falsch(teil, label_feld):\n    mehrheit = mehrheitsklasse(teil, label_feld)\n    falsch = 0\n    for beispiel in teil:\n        if beispiel[label_feld] != mehrheit:\n            falsch += 1\n    return falsch\n\ndef bester_schwellenwert(daten, merkmal, label_feld):\n    werte = sorted(set(b[merkmal] for b in daten))\n    beste_schwelle = None\n    wenigste_fehler = None\n    for i in range(len(werte) - 1):\n        kandidat = (werte[i] + werte[i + 1]) / 2\n        links, rechts = teile_bei_schwellenwert(daten, merkmal, kandidat)\n        fehler = anzahl_falsch(links, label_feld) + anzahl_falsch(rechts, label_feld)\n        if wenigste_fehler is None or fehler < wenigste_fehler:\n            wenigste_fehler = fehler\n            beste_schwelle = kandidat\n    return beste_schwelle, wenigste_fehler\n\ndef baue_baum(daten, merkmal, label_feld):\n    schwelle, _ = bester_schwellenwert(daten, merkmal, label_feld)\n    links, rechts = teile_bei_schwellenwert(daten, merkmal, schwelle)\n    return {\n        "merkmal": merkmal,\n        "schwelle": schwelle,\n        "links": mehrheitsklasse(links, label_feld),\n        "rechts": mehrheitsklasse(rechts, label_feld),\n    }\n\ndef klassifiziere(baum, beispiel):\n    if beispiel[baum["merkmal"]] < baum["schwelle"]:\n        return baum["links"]\n    return baum["rechts"]\n\nnachrichten = [\n    {"grossbuchstaben_anteil": 4, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 8, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 12, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 16, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 20, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 24, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 28, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 32, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 18, "kategorie": "spam"},\n    {"grossbuchstaben_anteil": 22, "kategorie": "spam"},\n]\nbaum = baue_baum(nachrichten, "grossbuchstaben_anteil", "kategorie")\nprint(baum)\nprint(genauigkeit_je_klasse(nachrichten, "kategorie", lambda b: klassifiziere(baum, b)))\n']);
    await page.locator('.btn-next').click();
    await expect(page.locator('.progress-count')).toContainText('3 abgeschlossen');

    // Lektion 4: Ausgewogene Daten dagegen.
    await solve(page, [null, 'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\ndef teile_bei_schwellenwert(daten, merkmal, schwelle):\n    links = [b for b in daten if b[merkmal] < schwelle]\n    rechts = [b for b in daten if b[merkmal] >= schwelle]\n    return links, rechts\n\ndef anzahl_falsch(teil, label_feld):\n    mehrheit = mehrheitsklasse(teil, label_feld)\n    falsch = 0\n    for beispiel in teil:\n        if beispiel[label_feld] != mehrheit:\n            falsch += 1\n    return falsch\n\ndef bester_schwellenwert(daten, merkmal, label_feld):\n    werte = sorted(set(b[merkmal] for b in daten))\n    beste_schwelle = None\n    wenigste_fehler = None\n    for i in range(len(werte) - 1):\n        kandidat = (werte[i] + werte[i + 1]) / 2\n        links, rechts = teile_bei_schwellenwert(daten, merkmal, kandidat)\n        fehler = anzahl_falsch(links, label_feld) + anzahl_falsch(rechts, label_feld)\n        if wenigste_fehler is None or fehler < wenigste_fehler:\n            wenigste_fehler = fehler\n            beste_schwelle = kandidat\n    return beste_schwelle, wenigste_fehler\n\ndef baue_baum(daten, merkmal, label_feld):\n    schwelle, _ = bester_schwellenwert(daten, merkmal, label_feld)\n    links, rechts = teile_bei_schwellenwert(daten, merkmal, schwelle)\n    return {\n        "merkmal": merkmal,\n        "schwelle": schwelle,\n        "links": mehrheitsklasse(links, label_feld),\n        "rechts": mehrheitsklasse(rechts, label_feld),\n    }\n\ndef klassifiziere(baum, beispiel):\n    if beispiel[baum["merkmal"]] < baum["schwelle"]:\n        return baum["links"]\n    return baum["rechts"]\n\nnachrichten = [\n    {"grossbuchstaben_anteil": 4, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 8, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 12, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 16, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 20, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 24, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 28, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 32, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 18, "kategorie": "spam"},\n    {"grossbuchstaben_anteil": 22, "kategorie": "spam"},\n]\nnachrichten_ausgewogen = [\n    {"grossbuchstaben_anteil": 4, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 8, "kategorie": "kein_spam"},\n    {"grossbuchstaben_anteil": 18, "kategorie": "spam"},\n    {"grossbuchstaben_anteil": 22, "kategorie": "spam"},\n]\nbaum2 = baue_baum(nachrichten_ausgewogen, "grossbuchstaben_anteil", "kategorie")\nprint(baum2)\nprint(genauigkeit_je_klasse(nachrichten, "kategorie", lambda b: klassifiziere(baum2, b)))\n']);
    await page.locator('.btn-next').click();
    await expect(page.locator('.progress-count')).toContainText('4 abgeschlossen');

    // Lektion 5: Grenzen von KI: Chatbots und Datenschutz.
    await solve(page, [null, 'def anonymisiere(profil):\n    return {\n        "name": None,\n        "email": None,\n        "alter": profil["alter"],\n        "plz": profil["plz"],\n    }\n\nprofile = [\n    {"name": "Mia", "email": "mia@example.com", "alter": 13, "plz": "10115"},\n    {"name": "Ben", "email": "ben@example.com", "alter": 15, "plz": "80331"},\n]\nfor profil in profile:\n    print(anonymisiere(profil))\n']);
    await expect(page.locator('.progress-count')).toContainText('5 abgeschlossen');
    await expect(page.locator('.lesson-complete-box')).toBeVisible();
    await page.locator('.btn-next').click();
    ...
    // Mission: eigener Bewertungs-Datensatz, Bias zeigen, dann ausgleichen.
    await solve(page, [
      'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\nbewertungen = [\n    {"wortanzahl": 5, "beschwerde": "nein"},\n    {"wortanzahl": 10, "beschwerde": "nein"},\n    {"wortanzahl": 15, "beschwerde": "nein"},\n    {"wortanzahl": 20, "beschwerde": "nein"},\n    {"wortanzahl": 25, "beschwerde": "nein"},\n    {"wortanzahl": 30, "beschwerde": "nein"},\n    {"wortanzahl": 35, "beschwerde": "nein"},\n    {"wortanzahl": 40, "beschwerde": "nein"},\n    {"wortanzahl": 22, "beschwerde": "ja"},\n    {"wortanzahl": 27, "beschwerde": "ja"},\n]\nprint(anzahl_je_klasse(bewertungen, "beschwerde"))\n',
      'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\nbewertungen = [\n    {"wortanzahl": 5, "beschwerde": "nein"},\n    {"wortanzahl": 10, "beschwerde": "nein"},\n    {"wortanzahl": 15, "beschwerde": "nein"},\n    {"wortanzahl": 20, "beschwerde": "nein"},\n    {"wortanzahl": 25, "beschwerde": "nein"},\n    {"wortanzahl": 30, "beschwerde": "nein"},\n    {"wortanzahl": 35, "beschwerde": "nein"},\n    {"wortanzahl": 40, "beschwerde": "nein"},\n    {"wortanzahl": 22, "beschwerde": "ja"},\n    {"wortanzahl": 27, "beschwerde": "ja"},\n]\nbasis = mehrheitsklasse(bewertungen, "beschwerde")\nprint(genauigkeit_je_klasse(bewertungen, "beschwerde", lambda b: basis))\n',
      'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\ndef teile_bei_schwellenwert(daten, merkmal, schwelle):\n    links = [b for b in daten if b[merkmal] < schwelle]\n    rechts = [b for b in daten if b[merkmal] >= schwelle]\n    return links, rechts\n\ndef anzahl_falsch(teil, label_feld):\n    mehrheit = mehrheitsklasse(teil, label_feld)\n    falsch = 0\n    for beispiel in teil:\n        if beispiel[label_feld] != mehrheit:\n            falsch += 1\n    return falsch\n\ndef bester_schwellenwert(daten, merkmal, label_feld):\n    werte = sorted(set(b[merkmal] for b in daten))\n    beste_schwelle = None\n    wenigste_fehler = None\n    for i in range(len(werte) - 1):\n        kandidat = (werte[i] + werte[i + 1]) / 2\n        links, rechts = teile_bei_schwellenwert(daten, merkmal, kandidat)\n        fehler = anzahl_falsch(links, label_feld) + anzahl_falsch(rechts, label_feld)\n        if wenigste_fehler is None or fehler < wenigste_fehler:\n            wenigste_fehler = fehler\n            beste_schwelle = kandidat\n    return beste_schwelle, wenigste_fehler\n\ndef baue_baum(daten, merkmal, label_feld):\n    schwelle, _ = bester_schwellenwert(daten, merkmal, label_feld)\n    links, rechts = teile_bei_schwellenwert(daten, merkmal, schwelle)\n    return {\n        "merkmal": merkmal,\n        "schwelle": schwelle,\n        "links": mehrheitsklasse(links, label_feld),\n        "rechts": mehrheitsklasse(rechts, label_feld),\n    }\n\ndef klassifiziere(baum, beispiel):\n    if beispiel[baum["merkmal"]] < baum["schwelle"]:\n        return baum["links"]\n    return baum["rechts"]\n\nbewertungen = [\n    {"wortanzahl": 5, "beschwerde": "nein"},\n    {"wortanzahl": 10, "beschwerde": "nein"},\n    {"wortanzahl": 15, "beschwerde": "nein"},\n    {"wortanzahl": 20, "beschwerde": "nein"},\n    {"wortanzahl": 25, "beschwerde": "nein"},\n    {"wortanzahl": 30, "beschwerde": "nein"},\n    {"wortanzahl": 35, "beschwerde": "nein"},\n    {"wortanzahl": 40, "beschwerde": "nein"},\n    {"wortanzahl": 22, "beschwerde": "ja"},\n    {"wortanzahl": 27, "beschwerde": "ja"},\n]\nbaum = baue_baum(bewertungen, "wortanzahl", "beschwerde")\nprint(genauigkeit_je_klasse(bewertungen, "beschwerde", lambda b: klassifiziere(baum, b)))\nbewertungen_ausgewogen = [\n    {"wortanzahl": 5, "beschwerde": "nein"},\n    {"wortanzahl": 10, "beschwerde": "nein"},\n    {"wortanzahl": 22, "beschwerde": "ja"},\n    {"wortanzahl": 27, "beschwerde": "ja"},\n]\nbaum2 = baue_baum(bewertungen_ausgewogen, "wortanzahl", "beschwerde")\nprint(genauigkeit_je_klasse(bewertungen, "beschwerde", lambda b: klassifiziere(baum2, b)))\n',
    ]);
    await expect(page.locator('.progress-count')).toContainText('7 abgeschlossen');
    ...
    // Boss 1: wenn eine von drei Klassen fast verschwindet.
    await solve(page, [
      'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\nprioritaet = (\n    [{"wartezeit": w, "stufe": "niedrig"} for w in [5, 8, 11, 14, 17, 20]]\n    + [{"wartezeit": w, "stufe": "mittel"} for w in [25, 28, 31]]\n    + [{"wartezeit": 60, "stufe": "hoch"}]\n)\nprint(anzahl_je_klasse(prioritaet, "stufe"))\n',
      'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\nprioritaet = (\n    [{"wartezeit": w, "stufe": "niedrig"} for w in [5, 8, 11, 14, 17, 20]]\n    + [{"wartezeit": w, "stufe": "mittel"} for w in [25, 28, 31]]\n    + [{"wartezeit": 60, "stufe": "hoch"}]\n)\nbasis = mehrheitsklasse(prioritaet, "stufe")\nprint(basis)\nprint(genauigkeit([basis for _ in prioritaet], [b["stufe"] for b in prioritaet]))\n',
      'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\nprioritaet = (\n    [{"wartezeit": w, "stufe": "niedrig"} for w in [5, 8, 11, 14, 17, 20]]\n    + [{"wartezeit": w, "stufe": "mittel"} for w in [25, 28, 31]]\n    + [{"wartezeit": 60, "stufe": "hoch"}]\n)\nbasis = mehrheitsklasse(prioritaet, "stufe")\nprint(genauigkeit_je_klasse(prioritaet, "stufe", lambda b: basis))\n',
    ]);
    await page.locator('.btn-next').click();

    // Boss 2: Bias beheben durch Duplizieren statt Wegschneiden (Oversampling).
    await solve(page, [
      'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\ntickets = [\n    {"wartezeit": 5, "dringend": "nein"},\n    {"wartezeit": 10, "dringend": "nein"},\n    {"wartezeit": 15, "dringend": "nein"},\n    {"wartezeit": 20, "dringend": "nein"},\n    {"wartezeit": 25, "dringend": "nein"},\n    {"wartezeit": 30, "dringend": "nein"},\n    {"wartezeit": 35, "dringend": "nein"},\n    {"wartezeit": 40, "dringend": "nein"},\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n]\ntickets_oversampled = tickets + [\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n]\nprint(anzahl_je_klasse(tickets_oversampled, "dringend"))\n',
      'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\ndef teile_bei_schwellenwert(daten, merkmal, schwelle):\n    links = [b for b in daten if b[merkmal] < schwelle]\n    rechts = [b for b in daten if b[merkmal] >= schwelle]\n    return links, rechts\n\ndef anzahl_falsch(teil, label_feld):\n    mehrheit = mehrheitsklasse(teil, label_feld)\n    falsch = 0\n    for beispiel in teil:\n        if beispiel[label_feld] != mehrheit:\n            falsch += 1\n    return falsch\n\ndef bester_schwellenwert(daten, merkmal, label_feld):\n    werte = sorted(set(b[merkmal] for b in daten))\n    beste_schwelle = None\n    wenigste_fehler = None\n    for i in range(len(werte) - 1):\n        kandidat = (werte[i] + werte[i + 1]) / 2\n        links, rechts = teile_bei_schwellenwert(daten, merkmal, kandidat)\n        fehler = anzahl_falsch(links, label_feld) + anzahl_falsch(rechts, label_feld)\n        if wenigste_fehler is None or fehler < wenigste_fehler:\n            wenigste_fehler = fehler\n            beste_schwelle = kandidat\n    return beste_schwelle, wenigste_fehler\n\ndef baue_baum(daten, merkmal, label_feld):\n    schwelle, _ = bester_schwellenwert(daten, merkmal, label_feld)\n    links, rechts = teile_bei_schwellenwert(daten, merkmal, schwelle)\n    return {\n        "merkmal": merkmal,\n        "schwelle": schwelle,\n        "links": mehrheitsklasse(links, label_feld),\n        "rechts": mehrheitsklasse(rechts, label_feld),\n    }\n\ndef klassifiziere(baum, beispiel):\n    if beispiel[baum["merkmal"]] < baum["schwelle"]:\n        return baum["links"]\n    return baum["rechts"]\n\ntickets = [\n    {"wartezeit": 5, "dringend": "nein"},\n    {"wartezeit": 10, "dringend": "nein"},\n    {"wartezeit": 15, "dringend": "nein"},\n    {"wartezeit": 20, "dringend": "nein"},\n    {"wartezeit": 25, "dringend": "nein"},\n    {"wartezeit": 30, "dringend": "nein"},\n    {"wartezeit": 35, "dringend": "nein"},\n    {"wartezeit": 40, "dringend": "nein"},\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n]\ntickets_oversampled = tickets + [\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n]\nbaum = baue_baum(tickets_oversampled, "wartezeit", "dringend")\nprint(baum)\n',
      'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\ndef teile_bei_schwellenwert(daten, merkmal, schwelle):\n    links = [b for b in daten if b[merkmal] < schwelle]\n    rechts = [b for b in daten if b[merkmal] >= schwelle]\n    return links, rechts\n\ndef anzahl_falsch(teil, label_feld):\n    mehrheit = mehrheitsklasse(teil, label_feld)\n    falsch = 0\n    for beispiel in teil:\n        if beispiel[label_feld] != mehrheit:\n            falsch += 1\n    return falsch\n\ndef bester_schwellenwert(daten, merkmal, label_feld):\n    werte = sorted(set(b[merkmal] for b in daten))\n    beste_schwelle = None\n    wenigste_fehler = None\n    for i in range(len(werte) - 1):\n        kandidat = (werte[i] + werte[i + 1]) / 2\n        links, rechts = teile_bei_schwellenwert(daten, merkmal, kandidat)\n        fehler = anzahl_falsch(links, label_feld) + anzahl_falsch(rechts, label_feld)\n        if wenigste_fehler is None or fehler < wenigste_fehler:\n            wenigste_fehler = fehler\n            beste_schwelle = kandidat\n    return beste_schwelle, wenigste_fehler\n\ndef baue_baum(daten, merkmal, label_feld):\n    schwelle, _ = bester_schwellenwert(daten, merkmal, label_feld)\n    links, rechts = teile_bei_schwellenwert(daten, merkmal, schwelle)\n    return {\n        "merkmal": merkmal,\n        "schwelle": schwelle,\n        "links": mehrheitsklasse(links, label_feld),\n        "rechts": mehrheitsklasse(rechts, label_feld),\n    }\n\ndef klassifiziere(baum, beispiel):\n    if beispiel[baum["merkmal"]] < baum["schwelle"]:\n        return baum["links"]\n    return baum["rechts"]\n\ntickets = [\n    {"wartezeit": 5, "dringend": "nein"},\n    {"wartezeit": 10, "dringend": "nein"},\n    {"wartezeit": 15, "dringend": "nein"},\n    {"wartezeit": 20, "dringend": "nein"},\n    {"wartezeit": 25, "dringend": "nein"},\n    {"wartezeit": 30, "dringend": "nein"},\n    {"wartezeit": 35, "dringend": "nein"},\n    {"wartezeit": 40, "dringend": "nein"},\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n]\ntickets_oversampled = tickets + [\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 22, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n    {"wartezeit": 27, "dringend": "ja"},\n]\nbaum = baue_baum(tickets_oversampled, "wartezeit", "dringend")\nprint(genauigkeit_je_klasse(tickets, "dringend", lambda b: klassifiziere(baum, b)))\n',
    ]);
    await page.locator('.btn-next').click();

    // Boss 3: wie gut schuetzt Anonymisierung wirklich.
    await solve(page, [
      'def gruppenzaehler(profile, felder):\n    anzahl = {}\n    for eintrag in profile:\n        schluessel = tuple(eintrag[feld] for feld in felder)\n        anzahl[schluessel] = anzahl.get(schluessel, 0) + 1\n    return anzahl\n\ndef anzahl_eindeutiger(anzahl):\n    eindeutige = 0\n    for schluessel in anzahl:\n        if anzahl[schluessel] == 1:\n            eindeutige += 1\n    return eindeutige\n\nprofile = [\n    {"plz": "10115", "alter": 13},\n    {"plz": "10115", "alter": 15},\n    {"plz": "80331", "alter": 13},\n    {"plz": "10115", "alter": 13},\n    {"plz": "80331", "alter": 17},\n    {"plz": "10115", "alter": 16},\n]\nprint(anzahl_eindeutiger(gruppenzaehler(profile, ["plz", "alter"])))\n',
      'def gruppenzaehler(profile, felder):\n    anzahl = {}\n    for eintrag in profile:\n        schluessel = tuple(eintrag[feld] for feld in felder)\n        anzahl[schluessel] = anzahl.get(schluessel, 0) + 1\n    return anzahl\n\ndef anzahl_eindeutiger(anzahl):\n    eindeutige = 0\n    for schluessel in anzahl:\n        if anzahl[schluessel] == 1:\n            eindeutige += 1\n    return eindeutige\n\nprofile = [\n    {"plz": "10115", "alter": 13},\n    {"plz": "10115", "alter": 15},\n    {"plz": "80331", "alter": 13},\n    {"plz": "10115", "alter": 13},\n    {"plz": "80331", "alter": 17},\n    {"plz": "10115", "alter": 16},\n]\ndef altersgruppe(alter):\n    start = (alter // 10) * 10\n    return f"{start}-{start + 9}"\n\nprofile_verallgemeinert = [\n    {"plz": p["plz"], "altersgruppe": altersgruppe(p["alter"])} for p in profile\n]\nprint(anzahl_eindeutiger(gruppenzaehler(profile_verallgemeinert, ["plz", "altersgruppe"])))\n',
      'def gruppenzaehler(profile, felder):\n    anzahl = {}\n    for eintrag in profile:\n        schluessel = tuple(eintrag[feld] for feld in felder)\n        anzahl[schluessel] = anzahl.get(schluessel, 0) + 1\n    return anzahl\n\ndef anzahl_eindeutiger(anzahl):\n    eindeutige = 0\n    for schluessel in anzahl:\n        if anzahl[schluessel] == 1:\n            eindeutige += 1\n    return eindeutige\n\nprofile = [\n    {"plz": "10115", "alter": 13},\n    {"plz": "10115", "alter": 15},\n    {"plz": "80331", "alter": 13},\n    {"plz": "10115", "alter": 13},\n    {"plz": "80331", "alter": 17},\n    {"plz": "10115", "alter": 16},\n]\ndef altersgruppe(alter):\n    start = (alter // 10) * 10\n    return f"{start}-{start + 9}"\n\nprofile_verallgemeinert = [\n    {"plz": p["plz"], "altersgruppe": altersgruppe(p["alter"])} for p in profile\n]\nprint(anzahl_eindeutiger(gruppenzaehler(profile, ["plz", "alter"])))\nprint(anzahl_eindeutiger(gruppenzaehler(profile_verallgemeinert, ["plz", "altersgruppe"])))\n',
    ]);
    // Letzte Lektion (boss-03) abgeschlossen, kein weiterer Boss -> "Weiter" springt direkt zum
    // Wochen-Check (kein Mission->Boss-Uebergang mehr, also keine erneute Wahl-Seite).
    ...
    await passWeekQuiz(page);
    await passCodingChallenge(page, 0, 'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\ndaten = [{"x": 1, "label": "a"}, {"x": 2, "label": "a"}, {"x": 3, "label": "a"}, {"x": 4, "label": "b"}]\nprint(anzahl_je_klasse(daten, "label"))\nprint(mehrheitsklasse(daten, "label"))\n');
    await passCodingChallenge(
      page,
      1,
      'def anzahl_je_klasse(daten, label_feld):\n    anzahl = {}\n    for beispiel in daten:\n        anzahl[beispiel[label_feld]] = anzahl.get(beispiel[label_feld], 0) + 1\n    return anzahl\n\ndef mehrheitsklasse(daten, label_feld):\n    anzahl = anzahl_je_klasse(daten, label_feld)\n    beste_klasse = None\n    bester_wert = -1\n    for klasse in anzahl:\n        if anzahl[klasse] > bester_wert:\n            bester_wert = anzahl[klasse]\n            beste_klasse = klasse\n    return beste_klasse\n\ndef genauigkeit(vorhersagen, erwartete_werte):\n    richtig = 0\n    for i in range(len(vorhersagen)):\n        if vorhersagen[i] == erwartete_werte[i]:\n            richtig += 1\n    return round(richtig / len(vorhersagen) * 100)\n\ndef genauigkeit_je_klasse(daten, label_feld, vorhersage_funktion):\n    ergebnis = {}\n    for klasse in anzahl_je_klasse(daten, label_feld):\n        teil = [b for b in daten if b[label_feld] == klasse]\n        vorhersagen = [vorhersage_funktion(b) for b in teil]\n        erwartete_werte = [klasse for _ in teil]\n        ergebnis[klasse] = genauigkeit(vorhersagen, erwartete_werte)\n    return ergebnis\n\ndaten2 = [{"label": "a"}, {"label": "a"}, {"label": "b"}]\nprint(genauigkeit_je_klasse(daten2, "label", lambda b: "a"))\n'
    );

    await expect(page.locator('.certificate-reveal')).toBeVisible({ timeout: 10000 });
    // Letzte Woche des Kurses -> kein "naechste Woche"-Button mehr (Woche 9 existiert nicht),
    // nur noch die Option, zur Wochenuebersicht zurueckzukehren.
    await expect(page.locator('.after-check-choice .after-check-tile')).toHaveCount(1);
  });
});
