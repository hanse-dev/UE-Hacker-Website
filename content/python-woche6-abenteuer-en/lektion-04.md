# ➖ Collection Spell 4: Removing entries

```python
treasures = ["Gold", "Crystal", "Amulet", "Crown", "Ring"]
treasures.remove("Crown")      # removes the first entry with this value
last = treasures.pop()          # removes the LAST one and returns it
first = treasures.pop(0)        # removes the entry at index 0 and returns it
treasures.clear()               # empties the whole list
```
