...
> **Aktueller Stand:** Branch `kurs-ki-labor` (noch nicht gemergt): Woche 2 "Daten sind alles"
> fertig und getestet (3.68) — 5 Lektionen (Datensatz als Liste von Dictionaries, mehrere Merkmale
> pro Beispiel, Muster per List Comprehension filtern, `.get()` mit Standardwert bei fehlenden
> Werten, Merkmale/Label trennen) + Debug (3 Bugs) + Mission + 3 Extra-Herausforderungen + eigener
> Wochen-Check (`content/ki-labor-checks/week-2.json`), Boss-2 baut bewusst schon den
> Merkmalsvergleich vor, der in Woche 3 (k-NN) gebraucht wird. Wochen 3–8 offen, nächste Session
> macht hier weiter. Vorheriger Stand: Grundgerüst (Wochenauswahl, `kurse.json`, Routing) + Woche 1
> "Was ist KI?" fertig und getestet (3.66); Quiz+Zertifikat + Extra-Herausforderung (3
> Boss-Lektionen) auf Nutzerwunsch nachgezogen (3.67) — das bislang python-only Check-System dafür
> kurs-fähig gemacht (`useWeekChecks.js`/`WeekCheckPanel.vue`/`CodeChallenge.vue`/
> `useCertificatePdf.js` um `courseKey` erweitert).
