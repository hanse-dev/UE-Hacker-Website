...
|------|---------|--------|
| **Inheritance** | A class inherits all properties and methods from another | `class Mage(Hero):` |
| `super()` | Access the parent class | `super().__init__(name)` |
| **Polymorphism *(= same method that works differently depending on the class)*** | Same method – different behaviour depending on the class | `animal.make_sound()` – Dog: Woof, Cat: Meow |
