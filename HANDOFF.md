...
> Zwei weitere Branches offen, noch nicht gemergt: `projekt-caesar-eigener-code` (3.61, Cäsar auf
> selbst schreiben + Lösung auf Wunsch umgestellt) und `projekt-abschluss-badge-link` (3.62,
> Abschluss-Screen der letzten Projekt-Lektion zeigt jetzt Abzeichen-Hinweis + Link zu /projekte).
> `npm run test:checks` grün (346 Tests auf `projekt-abschluss-badge-link`).
