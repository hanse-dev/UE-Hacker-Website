# 🚀 System-Protokoll 3: Methoden und self
...
2. Beim Aufruf schreibst du **`roboter.stelle_vor()`** – das `self` setzt Python selbst ein
3. Mit **`self.name`** greift die Methode auf die Attribute **dieses** Objekts zu

> 💡 Python übersetzt `roboter.stelle_vor()` in `Roboter.stelle_vor(roboter)`.
