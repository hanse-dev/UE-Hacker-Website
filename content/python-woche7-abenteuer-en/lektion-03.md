# 🃏 Archive Spell 3: Choosing and shuffling at random
...
one = random.choice(shelves)          # one random entry
three = random.sample(shelves, 3)     # 3 different entries (no repeats)
mix = shelves.copy()
