"""# 📖 Glossary – 🐴 Week 6 – Lists: The Training Collections
...
| `.count()` | How often an element appears | `list_.count(\"x\")` |
| `in` | Check if an element is in the list | `\"x\" in list_` |
| `.sort()` | Sort the list in ascending order (modifies the list) | `list_.sort()` |
| `sorted()` | Returns a sorted copy (original stays unchanged) | `sorted(list_)` |
...
| `break` | Exit a loop immediately | `if x == 5: break` |
| `continue` | Skip the current iteration and continue with the next | `if x == 0: continue` |
| `set` | A collection – like a list but without duplicates and without a fixed order | `set([1,1,2])` → `{1, 2}` |
